...
func (r *SQLiteRepository) CreateChunk(chunk *model.DocumentChunk) (int64, error) {
	...
	result, err := r.db.Exec(`
		INSERT INTO document_chunks (doc_id, content, chunk_index, metadata, vector_id)
		VALUES (?, ?, ?, ?, ?)
	`, chunk.DocID, chunk.Content, chunk.ChunkIndex, string(metadata), chunk.VectorID)
	if err != nil {
...
func (r *SQLiteRepository) GetChunksByDocID(docID int64) ([]model.DocumentChunk, error) {
	rows, err := r.db.Query(`
		SELECT id, doc_id, content, chunk_index, metadata, vector_id
	...
	for rows.Next() {
		...
		rows.Scan(&chunk.ID, &chunk.DocID, &chunk.Content, &chunk.ChunkIndex, &metadataJSON, &chunk.VectorID)
...
// CreateSession 创建会话
func (r *SQLiteRepository) CreateSession(sessionID string) error {
	_, err := r.db.Exec(`
...
func (r *SQLiteRepository) GetSession(sessionID string) (*model.ChatSession, error) {
	...
	`, sessionID).Scan(&session.ID, &session.SessionID, &session.Title, &session.CreatedAt, &session.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
